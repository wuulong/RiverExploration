...
## 版本資訊
*   **版本**：v2.1 (Relic & Deep-Time Modeling)
*   **日期**：2026-03-09
*   **狀態**：補完架構，導入「物件導向歷史 (OO-History)」與「史前生活模擬」方法論，強化遺跡導航與跨時空演繹流程。
...
*   **第 11 章：WalkGIS —— 流域的數位神經與集體共創**
    *   11.4 **[v2.1 增補] 厚數據標註與遺跡標級化實務**。

*   **第 12 章：邁向人、河、AI 的共生導航 (總結)**

*   **第 13 章：[v2.1 增補] 未來展望：邁向跨時空虛擬對合**
...
#### **Layer 2：知識圖譜與邏輯建模 (Knowledge Atlas Layer)**
這是最高階的「大腦中樞」。它不再只是儲存文字，而是儲存「解釋」。透過 AI 整合前面的資料，推導出地名的演進、河道的動力演變與開發邏輯。這是地圖最值錢的「厚數據」。
...
**這就是 v2.0 探索者的核心競爭力：你的觀察，是有歷史大數據支撐的深度解讀。** 下一節，我們將直接進入 `taiwan-history-atlas` 的實戰應用，教你如何引入這套現成的資料庫。


---

<!-- START_OF_FILE: Chapter_2_5.md -->
...
<!-- START_OF_FILE: Chapter_2_6.md -->
# 第 2 章：文獻與數位檔案的採掘 —— 專家級的知識底盤

## 2.6 超越文獻的硬證據：解析遺跡資料庫的數據維度

在流域探索中，我們不僅要學會「讀」資料，更要學會「看穿」資料庫表格背後的地景預言。本專案所使用的 `taiwan-history-atlas`（臺灣歷史地圖智庫）中的遺址資料表，並非只是點位的集合，它為探索者提供了數個關鍵的**「解碼維度」**。

### 1. 遺址：土地上最誠實的絕對座標
考古遺址（Relics）是人類適應地理環境留下的物理結晶。無論是垃圾堆（貝塚）、石牆還是生活層，它們所在的高程、坡度與離河距離，都反映了當時環境下的生存決策。

### 2. 資料結構與規模 (Schema & Statistics)
本專案使用的 `archaeological_sites` 資料表，不僅規模龐大，更具備嚴謹的權位等級。以下為智庫內部的數據構成：
...
*   **富化程度 (`enrichment_level`)**：顯示該數據是否已透過 AI 或深研 (Deep Research) 注入了厚數據。層級越高，代表該點位具備越完整的「Layer 3 演義劇本」。

#### **(5) 靈魂口袋：數據與文獻的勾稽**
*   **AI 專用 JSON (`for_ai` / `meta_data`)**：內含對出土文物的描述、與特定文獻（如《台灣通史》）的交叉索引。我們利用這些欄位，教導 AI 學習遺址的屬性，進而生成 5.5 章的模擬劇本。

### 4. 讀者實踐：如何「發動」數據
讀者在閱讀本書提供的數據清單時，應該嘗試進行以下聯想：
...
---
*當我們理解了這些數據維度，下一章我們將介紹如何將這些靜態表格，轉化為具備「繼承與覆寫」能力的物件導向地誌模型。*
...
<!-- START_OF_FILE: Chapter_3_3.md -->
## 3.3 [v2.0 增補] 時光羅盤與 HGIS —— 1920 空間錨定與座標修正技術

在河流探索的世界中，最令研究者感到困惑的往往不是找不到地點，而是「對不準」。當你試圖在現代地圖上尋找百年前的古渡口或聚落時，你需要的不是一張普通的地圖，而是一套**「時光羅盤」**。本節將深入探討如何利用 1920 年的行政邊界作為核心錨點，並結合中研院的權威圖資進行精確的空間對合。

### 1. 為什麼 1920 年是時空對合的「金級錨點」？
...
*   **鏈結力**：大多數的歷史史料（如《臺灣通史》的行政區紀錄、土地清丈資料）都能與這套系統完美勾稽。
...
在史前探索的操作中，我們將一個「史前家庭/部落」簡化為追求 **「能源平衡」** 的動態單元。這不僅僅是假設，而是我們用來推測遺址位置的核心演算法：

*   **能源平衡模型**：
    *   **輸入 (Resources)**：環境格網中的獲取率（如：河口魚類採集、丘陵塊莖）。
    *   **能耗 (Cost)**：維持生命的基本代謝，加上移動與加工的能量支出。
*   **遷徙演算法 (Migration Algorithm)**：
...
*   **數據洞察**：約 80% 的生存聚落（Rank 1-2）群聚在離古河道 300 公尺至 800 公尺的緩衝帶。
*   **地景意義**：這代表了史前族群在「取水便利」與「洪泛風險」之間，存在一個精確的 **「決策甜點區」**。

#### (2) 高程位能模型 (Altitude Potential)
*   **數據洞察**：在沖積平原上，遺址往往精準地座落在比周邊高出僅 1.5 到 2 公尺的「微高地」（如：沙丘、緩坡邊緣）。
...
*   **實證規模 (Scale)**：
    ...
    *   **產出成果**：正式產出並匯入 **696 個厚數據 POI** 至 WalkGIS 系統。
    *   **實例參考**：[曾文溪水利與生態探索地圖 (2026) 導覽介面](https://walkgis-544663807110.us-west1.run.app/?node=official&map=20260128_zengwen_exploration)
...
*   **技術修正筆記 (Technical Correction)**：
    在進行空間對合時，我們發現 1920 向量圖層與現代衛星影像存在約 **30 公尺** 的系統性位移。透過以「安定蘇厝長興宮」與「善化慶安宮」等百年古廟作為座標錨點，我們成功對 POI 進行了偏移補正，確保歷史聚落的核心位置能準確降落在現代地景中。
...
*   **[Prompt - 莫拉克災後重建路徑分析]**
    「請比較『日光小林』自主重建與『大愛園區』援建模式在空間功能上的具體差異。」
*   **探索撇步 (Tips)**：
    造訪舊鐵橋時，低頭看腳下的濕地，那是工業廢水轉生為生態棲地的現場。
...
*   **實證規模與生產力 (Scale & Productivity)**：
    ...
    *   **極速建模**：憑藉已封裝的 AI Skill，僅花費 **15 分鐘** 就完成了從空間萃取、史料厚化到 WalkGIS 正式匯入的完整閉環。
    *   **產出成果**：成功產出並匯入 **463 個厚數據 POI**。
...
*   **[v2.0 增補] 三層 Prompt 指令集心法**：
    設計指令時，應依照 **L0 (全文檢索)**、**L1 (實體萃取)** 到 **L2 (邏輯建模)** 的階梯進行。例如：「先查出所有關於二層行溪的原文 (L0)，萃取出聚落地標 (L1)，最後分析改道對歷史權力的影響 (L2)」。這確保了 AI 的洞察具備扎實的史料根據。


---

<!-- START_OF_FILE: Chapter_10_3.md -->
...
<!-- START_OF_FILE: Chapter_11_5.md -->
## 11.5 [v2.1 增補] 民間河川拓樸註冊表 (WRA-Civ) 與社群共創規範

隨者河流探索的深入，官方水利署（WRA）所定義 of 6 碼河川拓樸編碼（例如主流與二級支流的英數編碼 `114021`）在面對細部小支流、野溪或無名溪時，常面臨資料缺失的窘境。為此，我們推出了「官方圖資為底，民間協作補齊」的 **WRA-Civ（Civilian River Topology Registry）編碼與社群共創架構**，讓所有探索者都能一同描繪台灣細緻的水系網路。
