...
在實際運作中，LLM 解析 Wiki 水系列表時，只需要依據 Wiki 的清單縮排產出 `level` 數字即可，**不需要在 JSON 每一筆中指定父節點**，後續的父子親緣關係與程式碼完全交由 `river_topology_importer.py` 透過 Stack 堆疊演演演演演演演演演演演演演演演演演演演演演演演演演演算法精確還原。
