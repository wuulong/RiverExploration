...
- [x] **3.1 遺跡標級化實作**：定義 Rank 1-3 遺跡並建立與 HGIS 的對照整合規範。（已於 Ch 2.6 中詳述規範）
- [x] **3.2 資料結構與 `taiwan-history-atlas` 對照整合**：落實 `relic_master` 中 `for_ai` 與 `meta_data` 欄位的應用說明。
- [x] **3.3 一鍵產製 QGIS 專案**：將專用 SQL 查詢與 `.qgs` 渲染範本寫入附錄。（已於 Ch 10.4 說明流程並指向附錄集）
- [x] **3.4 Sidecars 規範建立**：落實 Markdown 與 DB 勾稽的技術細節說明。
- [x] **3.5 空間分析程式碼與 SOP**：提供產出上述分析範例的 Python/SQL 範本，確保讀者可重製。
